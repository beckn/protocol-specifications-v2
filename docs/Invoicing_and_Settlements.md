...
2. **Resolved (see [NFH-014](./Resolving_Disputes.md)).** A `DISPUTED` invoice is linked to its `Dispute` by a reference on the `Dispute`: a dispute `case` whose `subject` is a JSON-LD object with `@type: Invoice` (optionally identifying the contested entry) carries the invoice `id`. The `Invoice` schema is unchanged, keeping the linkage additive and one-directional.
