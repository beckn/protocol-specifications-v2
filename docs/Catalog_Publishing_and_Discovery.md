...
- **Id uniqueness within a catalog:** SHOULD ensure `Resource.id`/`Offer.id` values are unique within one catalog's own lineage (its `baseline` plus every `changes[]` entry applied so far). `beckn.yaml`'s own uniqueness guarantee is global, not catalog-scoped, so nothing else catches a PN reusing an id for two different resources or offers within its own single catalog.
...
**Rationale.**
- *Why intra-catalog id uniqueness matters:* a DS applies `resources.upserts`/`offers.upserts` by id, never by position (§Schema Changes). A PN that reuses an id for two different resources or offers within one catalog's own lineage will have one silently overwrite the other in every DS's index, with no protocol-level check catching it beforehand — hence a SHOULD, not a mechanism this RFC can otherwise enforce.
...
- MAY eventually drop a long-retired entry from the index entirely, as its own storage hygiene. This is a discoverability decision only: MUST NOT be taken to mean the underlying `resourceId`/`offerId` → object mapping can be fully discarded. Transactions or settlements initiated while the catalog was ACTIVE can complete, be disputed, or need resolution well after the catalog is retired, and a PN that has erased what an id referred to has no way to answer such a query. MUST retain enough of that mapping to resolve it.
...
- *Why a retired catalog's id/object mapping outlives the index entry:* dropping the index entry only removes a catalog from discovery — it says nothing about whether every transaction or settlement that referenced its ids while it was active has also concluded. The same "never destroy what might still be needed to answer a positive query" principle behind `retiredAt` itself applies here one layer deeper: to the identity data, not just the discoverability signal.
...
- The id/object-mapping retention this RFC requires for retired catalogs (CON-TBD-42) introduces no new PII beyond what `Catalog.provider` already carries; how long that retained mapping is kept is an operational retention-policy decision outside this RFC's schema scope.
...
| CON-TBD-41 | A PN SHOULD ensure `Resource.id`/`Offer.id` values are unique within one catalog's own lineage (its `baseline` plus every `changes[]` entry applied so far). | SHOULD |
| CON-TBD-42 | A PN MUST NOT fully discard a retired catalog's `resourceId`/`offerId` → object mapping merely because it has dropped the corresponding index entry; it MUST retain enough of that mapping to resolve transactions or settlements that reference those ids after retirement. | MUST NOT |
...
11. **Retired-catalog id/object-mapping retention duration (CON-TBD-42).** A PN MUST retain enough of a retired catalog's id/object mapping to resolve transactions or settlements initiated while it was active, but this RFC does not define a minimum or maximum retention window — that varies by network, domain, and applicable settlement/dispute timelines, and is out of scope for this RFC to set.
